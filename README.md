...
## 주요 내용
- **ALS 한계:** 정확도가 너무 안나와서 조사한 결과 ALS는 "사용자×아이템 **공발생(co‑occurrence)**"을 많이 볼수록 임베딩이 좋아지는데, 저희의 데이터에는 아티스트는 많지만 유저가 11명뿐이라 그래프가 **너무 희소**해서 모델이 아이템 간 관계를 풍부하게 못 배우는 것 같습니다.
