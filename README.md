...
## 1) Overview
본 프로젝트는 **Last.fm 재생 로그**와 **Spotify 메타/오디오 피처**를 결합하여  
**2-Stage 추천(ALS → LGBM LambdaRank 리랭커)**으로 유저별 아티스트를 추천하고,  
노트북에서 **tempo/valence 취향과 상황(컨텍스트)**을 반영해 **개인화 플레이리스트(트랙 단위)**를 생성합니다.
최종 결과는 간단한 **GUI**(노트북 위젯)로 출력하여 사용자의 이해를 돕습니다.
...
$$
\mathbf{FinalScore}=0.6\cdot \text{Base\_score}
+ 0.2\cdot \text{Tempo\_sim}
+ 0.2\cdot \text{Valence\_sim}
$$
