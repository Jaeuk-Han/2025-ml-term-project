...
## 진행한 내용 요약
- Last.fm 재생 로그 + Spotify 오디오/장르 피처를 합쳐서,
  1) **ALS(implicit)**로 사용자별 추천 **후보 200개**를 만들고  
  2) **LGBM 리랭커**로 **유저 프로필 × 아이템 피처**를 활용해 **순서를 재정렬**했습니다.
...
- **리랭커 효과:** 부족한 정확도를 보완하고자 콘텐츠 기반 피처(오디오/장르/인기도)와 유저 프로필(가중 평균 + 장르 Top‑K/엔트로피)을 써서 순서를 다시 매기는 리랭커 개념을 도입했습니다.(RAG에서의 개념과 유사) 결론적으로 **상위 랭크 품질(nDCG/MAP)이 크게 개선**되었음을 확인 가능했습니다.
...
**결과 해석**
- **α=0.5**에서 가장 균형이 좋았습니다.  
- **nDCG/MAP**이 크게 올라가 **“정답을 더 위로”** 끌어올리는 효과가 뚜렷했습니다.  
- **Precision@10≈0.52**(Top10 중 약 5개 정답)로, 유저 수 11명 대비 납득 가능한 수준으로 보입니다.
